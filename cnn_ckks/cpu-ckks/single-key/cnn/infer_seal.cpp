...
{
	double val;
	size_t num_c = 0, num_cb = 0, num_b = 0, num_m = 0, num_v = 0, num_w = 0;

	conv_weight.clear(); 
	conv_weight.resize(2);
	bn_bias.clear();
	bn_bias.resize(2);
	bn_running_mean.clear();
	bn_running_mean.resize(2);
	bn_running_var.clear();
	bn_running_var.resize(2);
	bn_weight.clear();
	bn_weight.resize(2);
	linear_weight.clear(); linear_bias.clear(); 
	linear_weight.resize(3); linear_bias.resize(3);
	...
	ci = 1, co = 6;
	for(long i=0; i<fh*fw*ci*co; i++) {val=0.1; conv_weight[num_c].emplace_back(val);} num_c++;
	ci = 6, co = 16;
	for(long i=0; i<fh*fw*ci*co; i++) {val=0.1; conv_weight[num_c].emplace_back(val);} num_c++;

	// bn parameters
	ci = 6;
	for(long i=0; i<ci; i++) {val=0.01; bn_bias[num_b].emplace_back(val);} num_b++;
	for(long i=0; i<ci; i++) {val=0.01; bn_running_mean[num_m].emplace_back(val);} num_m++;
	for(long i=0; i<ci; i++) {val=0.01; bn_running_var[num_v].emplace_back(val);} num_v++;
	for(long i=0; i<ci; i++) {val=0.01; bn_weight[num_w].emplace_back(val);} num_w++;
	ci = 16;
	for(long i=0; i<ci; i++) {val=0.01; bn_bias[num_b].emplace_back(val);} num_b++;
	for(long i=0; i<ci; i++) {val=0.01; bn_running_mean[num_m].emplace_back(val);} num_m++;
	for(long i=0; i<ci; i++) {val=0.01; bn_running_var[num_v].emplace_back(val);} num_v++;
	...
	// FC
	for(long i=0; i<256*128; i++) {val=0.01; linear_weight[num_cb].emplace_back(val);}
	for(long i=0; i<128; i++) {val=0.01; linear_bias[num_cb].emplace_back(val);} num_cb++;
	for(long i=0; i<128*84; i++) {val=0.01; linear_weight[num_cb].emplace_back(val); }
	for(long i=0; i<84; i++) {val=0.01; linear_bias[num_cb].emplace_back(val);} num_cb++;
	for(long i=0; i<84*10; i++) {val=0.01; linear_weight[num_cb].emplace_back(val);}
	for(long i=0; i<10; i++) {val=0.01; linear_bias[num_cb].emplace_back(val);}
}
...
void LeNet_mnist_seal_sparse(size_t layer_num, size_t start_image_id, size_t end_image_id)
{
	...
	ofstream out_share;
	out_share.open("../../result/lenet_mnist_label_" + to_string(start_image_id) + "_" + to_string(end_image_id));
	...
	int log_special_prime = 60;
    int log_integer_part = logq - logp - loge + 5;
	int remaining_level = 2; // 19; // Calculation required
	int boot_level = 10; //10; // greater than: subsum 1 + coefftoslot 2 + ModReduction 9 + slottocoeff 2 
	int total_level = remaining_level + boot_level;
	...
	for(size_t image_id = start_image_id; image_id <=end_image_id; image_id++)
	{
		...
		ofstream output;
		output.open("../../result/lenet_mnist_image" + to_string(image_id) + ".txt");
		string dir = "lenet" + to_string(layer_num) + "_new";
		...
		int co = 0, st = 0, fh = 5, fw = 5;
		long init_p = 8, n = 1<<logn;
		int stage = 0;
		double epsilon = 0.00001;
		vector<double> image;
		vector<vector<double>> linear_weight, linear_bias;
		vector<vector<double>> conv_weight, bn_bias, bn_running_mean, bn_running_var, bn_weight;

		std::cout << "Start import_parameters_mnist" << std::endl;
		import_parameters_mnist(linear_weight, linear_bias, conv_weight, bn_bias, bn_running_mean, bn_running_var, bn_weight, layer_num, end_num);
		std::cout << "End import_parameters_mnist" << std::endl;
		...
		for(long i=0; i<28*28*1 *image_id; i++) {in>>val;}
		for(long i=0; i<28*28*1; i++) {in>>val; image[i]=val;}  in.close();
		...
		// 24 24 6
		cnn = TensorCipher(logn, 1, 28, 28, 1, 1, init_p, image, encryptor, encoder, logq);
		...
		Ciphertext ctxt;
		ctxt = cnn.cipher();
		for(int i=0; i<boot_level-2; i++) evaluator.mod_switch_to_next_inplace(ctxt); 
		cnn.set_ciphertext(ctxt);

		cout << "\ncnn start level : " << context.get_context_data(cnn.cipher().parms_id())->chain_index() << endl;

		cout << "layer 0" << endl;
		output << "layer 0" << endl;
		siso_convolution_seal_print(cnn, cnn, 16, 1, 5, 5, conv_weight[0], encoder, encryptor, evaluator, gal_keys, cipher_pool, output, decryptor, context, 0, end_num);
		// repacked_convolution_seal_print(cnn, cnn, 6, 1, 5, 5, conv_weight[0], encoder, encryptor, evaluator, gal_keys, cipher_pool, output, decryptor, context, 0, end_num);
		...
		// // repacked_averagepooling_seal_scale_print(cnn, cnn, evaluator, gal_keys, B, output, decryptor, encoder, context);
		
		// cout << "layer 3" << endl;
		// output << "layer 3" << endl;
		// siso_convolution_seal_print(cnn, cnn, 16, 1, 5, 5, conv_weight[1], encoder, encryptor, evaluator, gal_keys, cipher_pool, output, decryptor, context, stage, end_num); stage++;
		// // repacked_convolution_seal_print(cnn, cnn, 16, 1, 5, 5, conv_weight[1], encoder, encryptor, evaluator, gal_keys, cipher_pool, output, decryptor, context, stage, end_num); stage++;

		// // modulus down
		// ctxt = cnn.cipher();
		// evaluator.rescale_to_next_inplace(ctxt);
		// for(int i=0; i<1; i++) evaluator.mod_switch_to_next_inplace(ctxt); // left for next repack
		// cnn.set_ciphertext(ctxt);
		// cout << "remaining level : " << context.get_context_data(ctxt.parms_id())->chain_index() << endl;
		// cout << "scale: " << ctxt.scale() << endl;
		...
		// cout << "layer 1" << endl;
		// output << "layer 1" << endl;
		// fully_connected_seal_print(cnn, cnn, linear_weight[1], linear_bias[1], 84, 128, evaluator, gal_keys, output, decryptor, encoder, context); stage++;
		...
		// cout << "layer 1" << endl;
		// output << "layer 1" << endl;
		// fully_connected_seal_print(cnn, cnn, linear_weight[2], linear_bias[2], 10, 84, evaluator, gal_keys, output, decryptor, encoder, context); stage++;
		...
		// Plaintext scaler;
		// double scale_change = pow(2.0,45) * ((double)modulus[cur_level-1].value()) / ctxt.scale();
