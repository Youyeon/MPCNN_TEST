...
void import_parameters_mnist(vector<vector<double>> &linear_weight, vector<vector<double>> &linear_bias, vector<vector<double>> &conv_weight, vector<vector<double>> &bn_bias, vector<vector<double>> &bn_running_mean, vector<vector<double>> &bn_running_var, vector<vector<double>> &bn_weight, size_t layer_num, size_t end_num)
{
	...
	// FC
	for(long i=0; i<256*128; i++) {val=0.1; linear_weight[num_cb].emplace_back(val); linear_bias[num_cb].emplace_back(val);} num_b++;
	for(long i=0; i<128*84; i++) {val=0.1; linear_weight[num_cb].emplace_back(val); linear_bias[num_cb].emplace_back(val);} num_b++;
	for(long i=0; i<84*10; i++) {val=0.1; linear_weight[num_cb].emplace_back(val); linear_bias[num_cb].emplace_back(val);} num_b++;
}
...
//	additional rotation kinds for CNN
	...
	for(size_t image_id = start_image_id; image_id <=end_image_id; image_id++)
	{
		...
		cout << "cnn remaining level : " << context.get_context_data(cnn.cipher().parms_id())->chain_index() << endl;
		// layer 0
		cout << "layer 0" << endl;
		output << "layer 0" << endl;
